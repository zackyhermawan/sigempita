...
<div class="max-w-2xl mx-auto px-2">
    ...
    <div class="bg-white rounded-2xl md:rounded-3xl p-6 md:p-10 shadow-sm border border-rose-50">
        ...
        <form action="/children/{{ $child->id }}" method="POST" class="space-y-5 md:space-y-6">
            @csrf
            @method('PUT')
            ...
            <button type="submit" class="w-full mt-4 py-4 md:py-5 bg-amber-400 text-white font-black rounded-xl md:rounded-2xl shadow-xl shadow-amber-100 hover:bg-amber-500 active:scale-[0.98] transition-all uppercase tracking-widest text-xs md:text-sm">
                Perbarui Data Si Kecil
            </button>
        </form>
    </div>
</div>
